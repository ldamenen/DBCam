// core/config.js
// Every tunable in one shared place (ARCHITECTURE §3). All clients — web, iOS,
// Android — read these; a change here lands on all three. Platform adapters must
// not carry their own numbers.
...
export const CONFIG = {
  ...
  detection: {
    ...
    delegate: 'CPU',
    // Detection assets are VENDORED (self-contained, ARCHITECTURE §0): the web
    // client ships MediaPipe's JS/wasm and both .tflite models in vendor/.
    // Remote URLs remain only as a fallback if local assets are missing.
    tasksVisionVersion: '0.10.14',
    localFaceModelPath: 'vendor/models/blaze_face_short_range.tflite',
    localObjectModelPath: 'vendor/models/efficientdet_lite0.tflite',
    faceModelUrl:
